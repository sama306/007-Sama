...
export function generateOrderPDF(order: Order): void {
  ...
  doc.text(
    'Este documento es un comprobante de tu pedido en 007-Sama.',
    doc.internal.pageSize.getWidth() / 2,
    footerY + 5,
    { align: 'center' },
  )
  doc.text(
    'Los pagos son procesados en modo test. Ningún cargo real fue realizado.',
    doc.internal.pageSize.getWidth() / 2,
    footerY + 9,
    { align: 'center' },
  )
  doc.text(
    'Para consultas: soporte@007sama.com',
    doc.internal.pageSize.getWidth() / 2,
    footerY + 13,
    { align: 'center' },
  )
